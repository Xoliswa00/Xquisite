...
<div class="px-6 py-5 space-y-5">
    ...
    @if (!$editing)
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Service Key <span class="text-red-500">*</span></label>
        <input type="text" name="key" value="{{ old('key') }}" placeholder="e.g. onboarding_standard"
               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
    </div>
    @else
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Service Key</label>
        <p class="font-mono text-sm text-gray-500 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg">{{ $service->key }}</p>
    </div>
    @endif
    ...
    <div>
        ...
        <textarea name="description" rows="3"
                  class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description', $service->description ?? '') }}</textarea>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Billing Type <span class="text-red-500">*</span></label>
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Price (ZAR)</label>
            <div class="relative">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Price Label Override</label>
            <input type="text" name="price_label" value="{{ old('price_label', $service->price_label ?? '') }}"
                   placeholder="e.g. From R800 or Custom quote"
                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            <p class="mt-1 text-xs text-gray-400">Overrides the price display. Leave blank to auto-format.</p>
        </div>
    </div>
    ...
    <div class="flex items-center gap-8 pt-1">
        <label class="flex items-center gap-3 cursor-pointer">
            ...
            <span class="text-sm text-gray-700">Active (visible to clients)</span>
        </label>
        <label class="flex items-center gap-3 cursor-pointer">
            ...
            <input type="checkbox" name="is_requestable" value="1"
                   ...
                   class="w-4 h-4 rounded text-indigo-600 border-gray-300">
            <span class="text-sm text-gray-700">Clients can self-request</span>
        </label>
    </div>
