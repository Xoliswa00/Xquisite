...
<div class="px-6 py-5 space-y-5">
    ...
    @if (!$editing)
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Plan Key <span class="text-red-500">*</span></label>
        <input type="text" name="key" value="{{ old('key') }}" placeholder="e.g. starter, growth, scale"
               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
        <p class="mt-1 text-xs text-gray-400">Lowercase, underscores only. Cannot be changed after creation.</p>
    </div>
    @else
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Plan Key</label>
        <p class="font-mono text-sm text-gray-500 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg">{{ $plan->key }}</p>
    </div>
    @endif
    ...
    <div>
        ...
        <textarea name="description" rows="2"
                  class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description', $plan->description ?? '') }}</textarea>
    </div>
    ...
    <div class="grid grid-cols-3 gap-4">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Annual Price (ZAR/mo)</label>
            <div class="relative">
                <span class="absolute inset-y-0 left-3 flex items-center text-gray-400 text-sm">R</span>
            ...
            <p class="mt-1 text-xs text-gray-400">Billed × 12 annually. Shows discount % on pricing page.</p>
        </div>
        <div>
    ...
    {{-- Modules --}}
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">Included Modules</label>
        <div class="grid grid-cols-2 gap-2">
            @foreach ($availableModules as $module)
            <label class="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-gray-200 hover:border-indigo-300 cursor-pointer transition">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">R{{ number_format($module->price, 0) }}/mo individually</p>
                </div>
            </label>
            @endforeach
        </div>
    ...
    <div class="flex items-center gap-8 pt-1">
        <label class="flex items-center gap-3 cursor-pointer">
            ...
            <input type="checkbox" name="is_active" value="1"
                   ...
                   class="w-4 h-4 rounded text-indigo-600 border-gray-300 focus:ring-indigo-500">
            <span class="text-sm text-gray-700">Active (visible on pricing page)</span>
        </label>
        ...
        <label class="flex items-center gap-3 cursor-pointer">
            ...
            <input type="checkbox" name="is_featured" value="1"
                   ...
                   class="w-4 h-4 rounded text-indigo-600 border-gray-300 focus:ring-indigo-500">
            <span class="text-sm text-gray-700">Featured (highlighted as recommended)</span>
        </label>
    </div>
