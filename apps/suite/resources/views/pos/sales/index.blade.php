    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="bg-slate-800 rounded-xl p-5">
                ...
                <p class="text-3xl font-bold text-white mt-1">R{{ number_format($todayTotal, 2) }}</p>
            </div>
            <div class="bg-slate-800 rounded-xl p-5">
        ...
        <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            <form method="GET" class="flex flex-wrap gap-2">
                <input type="text" name="search" value="{{ request('search') }}"
                       ...
                       class="bg-slate-800 border border-slate-700 text-slate-100 text-sm rounded-lg px-3 py-2 w-52 focus:outline-none focus:ring-1 focus:ring-indigo-500">
            ...
            <a href="{{ route('pos.terminal') }}"
               class="bg-emerald-600 hover:bg-emerald-500 text-white text-sm px-4 py-2 rounded-lg whitespace-nowrap">
                Open POS Terminal
            </a>
        </div>
                ...
                <tbody class="divide-y divide-slate-700">
                    @forelse($sales as $sale)
                        <tr class="hover:bg-slate-700/50">
                            ...
                            <td class="px-4 py-3">
                                <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-slate-700 text-slate-300">
