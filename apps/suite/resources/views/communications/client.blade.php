        ...
        <div class="flex gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1">
            <button @click="tab = 'platform'; window.location.hash = 'platform'"
                ...
                @if($platformMessages->where('is_from_owner', true)->whereNull('read_at')->count() > 0)
                    <span class="bg-indigo-600 text-white text-xs rounded-full px-1.5 py-0.5">{{ $platformMessages->where('is_from_owner', true)->whereNull('read_at')->count() }}</span>
                @endif
            </button>
                    ...
                    @if($msg->is_from_owner)
                        ...
                        <div class="flex justify-start">
                            <div class="max-w-[75%]">
                                ...
                                @endif
                                <div class="bg-indigo-900/40 border border-indigo-800/50 text-slate-200 rounded-2xl rounded-tl-none px-4 py-3 text-sm">
                                    {!! nl2br(e($msg->body)) !!}
                                </div>
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 mt-4">
                <form method="POST" action="{{ route('portal.messages.reply') }}" class="space-y-3">
                    ...
                    <textarea name="body" rows="3" required
                              class="w-full rounded-lg bg-slate-800 border-slate-700 text-slate-200 text-sm focus:ring-indigo-500 focus:border-indigo-500"
                              placeholder="Message Xquisite Support…"></textarea>
                    <x-input-error :messages="$errors->get('body')" />
                    <button type="submit" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg">Send</button>
                </form>
            </div>
        ...
        <div x-show="tab === 'clients'" id="clients">
            ...
            @if(!$hasClientMessaging)
                <div class="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center">
                    ...
                    <a href="{{ route('settings.modules.index') }}" class="mt-4 inline-block px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg">
                        View Modules
                    </a>
                </div>
            @elseif($clientThreads->isEmpty())
                <div class="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center">
                    <p class="text-slate-400">No clients yet. <a href="{{ route('clients.create') }}" class="text-indigo-400 hover:underline">Add your first client →</a></p>
                </div>
            @else
                    ...
                    @foreach($clientThreads as $client)
                        ...
                        <a href="{{ route('clients.messages', $client) }}"
                           class="flex items-center gap-4 px-5 py-4 border-b border-slate-800 last:border-0 hover:bg-slate-800/50 transition-colors">
                            <div class="w-9 h-9 rounded-full bg-indigo-700 flex items-center justify-center text-sm font-bold text-white shrink-0">
