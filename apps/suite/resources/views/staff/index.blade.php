        ...
        <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            <form method="GET" class="flex gap-2 flex-wrap">
                <input type="text" name="search" value="{{ request('search') }}"
                       ...
                       class="bg-slate-800 border border-slate-700 text-slate-100 text-sm rounded-lg px-3 py-2 w-full sm:w-48 focus:outline-none focus:ring-1 focus:ring-indigo-500">
            ...
            <div class="sm:hidden divide-y divide-slate-700">
                @forelse($staff as $member)
                    <a href="{{ route('staff.show', $member) }}" class="block px-4 py-3 hover:bg-slate-700/50 transition-colors">
                        <div class="flex items-center justify-between gap-3">
                            <p class="text-sm font-medium text-indigo-400">{{ $member->name }}</p>
                            @if($member->is_active)
                ...
                @empty
                    <div class="px-4 py-10 text-center text-slate-500 text-sm">
                        No staff yet. <a href="{{ route('staff.create') }}" class="text-indigo-400 hover:text-indigo-300">Add a member.</a>
                    </div>
                @endforelse
            </div>
