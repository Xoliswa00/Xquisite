...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4 text-sm">
            <div>
                ...
                <p class="text-slate-500">{{ $service->duration_minutes }} min Â· R{{ number_format($service->price, 2) }}</p>
            </div>
            <div>
                <p class="text-slate-400 text-xs uppercase font-semibold">Staff</p>
                <p class="font-semibold text-slate-900 mt-1">{{ $staff->name }}</p>
                @if($staff->role) <p class="text-slate-500">{{ $staff->role }}</p> @endif
            </div>
            <div class="col-span-2">
                ...
                <p class="text-slate-500">{{ $slot->format('H:i') }} â€“ {{ $slot->copy()->addMinutes($service->duration_minutes)->format('H:i') }}</p>
            </div>
        </div>
    </div>

    @if($customer)
        {{-- Already logged in â€” just submit --}}
        ...
        <form method="POST" action="{{ route('book.store', $slug) }}">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="notes" rows="3" placeholder="Any special requests or informationâ€¦"
    ...
    @else
        {{-- Not logged in â€” prompt to sign in or register --}}
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
            You need an account to complete your booking. Your slot is reserved while you sign in.
        </div>
