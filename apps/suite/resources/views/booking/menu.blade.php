<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto space-y-8">
        ...
        {{-- Hero --}}
        <div class="rounded-2xl bg-gradient-to-br from-slate-900 to-indigo-950 border border-slate-800 px-8 py-10">
            <h1 class="text-3xl font-bold text-white">Our Services</h1>
        ...
        <div>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                ...
                <div class="bg-slate-900 border border-indigo-800/60 rounded-xl overflow-hidden">
        ...
        <div>
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="flex gap-2 flex-wrap">
                    <a href="{{ route('booking.menu') }}"
                       class="px-3 py-1 rounded-full text-xs font-medium transition-colors {{ !$categoryFilter ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700' }}">
                        All
                    </a>
                    @foreach($categories as $cat)
                        <a href="{{ route('booking.menu', ['category' => $cat->id]) }}"
                           class="px-3 py-1 rounded-full text-xs font-medium transition-colors {{ $categoryFilter == $cat->id ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700' }}">
                            {{ $cat->icon }} {{ $cat->name }}
                        </a>
                    @endforeach
                    @if($uncategorized->count())
                        <a href="{{ route('booking.menu', ['category' => 'uncategorized']) }}"
                           class="px-3 py-1 rounded-full text-xs font-medium transition-colors {{ $categoryFilter === 'uncategorized' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700' }}">
                            Other
                        </a>
                    @endif
                </div>
            ...
            @if((!$categoryFilter || $categoryFilter === 'uncategorized') && $uncategorized->count())
                <div class="mb-8">
                    <h3 class="text-base font-semibold text-white mb-3">Other Services</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            @if($categories->every(fn($c) => $c->activeServices->isEmpty()) && $uncategorized->isEmpty())
                <div class="bg-slate-900 border border-slate-800 rounded-xl p-10 text-center text-slate-500 text-sm">
                    No active services yet. <a href="{{ route('services.create') }}" class="text-indigo-400 hover:text-indigo-300">Add one →</a>
                </div>
            @endif
        </div>
