        ...
        <form method="POST" action="{{ route('rental-orders.store') }}" class="bg-slate-800 rounded-xl p-6 space-y-5">
            ...
            <div>
                ...
                <select name="product_id" required @change="setProduct($event.target.value)"
                        class="w-full bg-slate-700 border border-slate-600 text-slate-100 text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#0078D4] @error('product_id') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="quantity" min="1" value="{{ old('quantity', 1) }}"
                           x-model.number="qty"
                           class="w-full bg-slate-700 border border-slate-600 text-slate-100 text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#0078D4] @error('quantity') border-red-500 @enderror">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1">Event Date <span class="text-red-400">*</span></label>
                    <input type="date" name="event_date" value="{{ old('event_date') }}" required
                           class="w-full bg-slate-700 border border-slate-600 text-slate-100 text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#0078D4] @error('event_date') border-red-500 @enderror">
                </div>
                <div>
                    ...
                    <input type="date" name="return_due_at" value="{{ old('return_due_at') }}" required
                           class="w-full bg-slate-700 border border-slate-600 text-slate-100 text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#0078D4] @error('return_due_at') border-red-500 @enderror">
                </div>
            </div>
