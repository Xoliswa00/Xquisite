<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        @php
            ...
            // Primary quick-action follows the tenant's active modules, in the same priority
            // order the sidebar renders its sections (layouts/app.blade.php): booking > pos > ecommerce > property > messaging.
            $primaryAction = null;
            if ($hasBooking) {
                $primaryAction = ['label' => 'New Booking', 'route' => route('appointments.create')];
            } elseif ($hasPos) {
                $primaryAction = ['label' => 'New Sale', 'route' => route('pos.terminal')];
            } elseif ($hasEcommerce) {
                $primaryAction = ['label' => 'Manage Store', 'route' => route('store.settings')];
            } elseif ($hasProperty) {
                $primaryAction = ['label' => 'Add Property', 'route' => route('properties.create')];
            } elseif ($hasMessaging) {
                $primaryAction = ['label' => 'View Clients', 'route' => route('clients.index')];
            }
        @endphp
        <div class="flex items-end justify-between gap-4">
            ...
            @if($primaryAction)
                <a href="{{ $primaryAction['route'] }}"
                   class="shrink-0 inline-flex items-center gap-1.5 px-3 py-2 sm:px-4 bg-[#0078D4] hover:bg-[#0065B8] text-white text-sm font-medium rounded-lg transition-colors">
        ...
        {{-- Pending module activation requests — admin only, independent of which modules are active --}}
        @can('manage-tenants')
            @if($pendingModuleRequests > 0)
                <a href="{{ route('admin.module-requests.index') }}"
                   class="flex items-center justify-between px-5 py-3 rounded-xl bg-[#0078D4]/10 border border-[#0078D4]/30 text-sm hover:bg-[#0078D4]/15 transition-colors">
                    <span class="text-[#0078D4]">{{ $pendingModuleRequests }} module {{ Str::plural('request', $pendingModuleRequests) }} awaiting review</span>
                    <span class="text-[#0078D4] font-medium">Review →</span>
                </a>
            @endif
        @endcan

        {{-- Onboarding checklist — steps drawn only from the tenant's active modules --}}
        @if(!$onboardingComplete)
            <div class="bg-slate-800 rounded-xl border border-[#0078D4]/20 shadow-lg shadow-[#0078D4]/8 overflow-hidden">
                <div class="px-5 py-4 border-b border-[#0078D4]/15 bg-gradient-to-r from-[#0078D4]/10 to-transparent flex items-center justify-between">
                    ...
                    <span class="text-xs text-[#0078D4] font-semibold bg-[#0078D4]/10 px-2.5 py-1 rounded-full border border-[#0078D4]/20">{{ collect($onboardingSteps)->filter(fn($s) => $s['done'])->count() }}/{{ count($onboardingSteps) }} done</span>
                </div>
                <div class="p-5 space-y-2">
                    @foreach($onboardingSteps as $step)
                        <div class="flex items-center gap-3 px-3 py-2 rounded-lg {{ $step['done'] ? 'opacity-50' : 'bg-slate-700/40' }}">
                            <div class="w-5 h-5 rounded-full shrink-0 flex items-center justify-center {{ $step['done'] ? 'bg-emerald-600' : 'border-2 border-slate-600' }}">
                                @if($step['done'])
                                    <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                                @endif
                            </div>
                            <span class="text-sm {{ $step['done'] ? 'line-through text-slate-500' : 'text-slate-300' }} flex-1">{{ $step['label'] }}</span>
                            @if(!$step['done'] && $step['route'])
                                <a href="{{ route($step['route']) }}" class="text-xs text-[#0078D4] hover:text-[#0065B8] shrink-0">Start →</a>
                            @endif
                        </div>
                    @endforeach
                </div>
        ...
        @if($hasBooking)
        <div>
            <h2 class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Bookings</h2>
        ...
        {{-- ══════════════════════ POS (standalone — only when Booking isn't active) ══════════════════════ --}}
        @if($hasPos && !$hasBooking)
        <div>
            <h2 class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Point of Sale</h2>

            <div class="bg-slate-800 rounded-xl border border-emerald-800/40 relative overflow-hidden shadow-lg shadow-emerald-950/40 mb-4">
                <div class="absolute inset-0 bg-gradient-to-br from-emerald-900/25 to-transparent pointer-events-none"></div>
                <div class="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-emerald-500/60 to-transparent"></div>
                <div class="relative p-5">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <p class="text-xs text-slate-400 uppercase tracking-wider">Till revenue this month</p>
                    </div>
                    <p class="stat-number text-4xl font-bold text-emerald-400 leading-none">R{{ number_format($posRevenueThisMonth, 2) }}</p>
                    <p class="text-xs text-slate-500 mt-2">paid sales</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <a href="{{ route('products.index') }}"
                   class="bg-slate-800 hover:bg-slate-700/70 rounded-xl p-4 transition-all shadow-md shadow-black/20 border border-slate-700/50 border-t-2 border-t-purple-500/40 group">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-xs text-slate-400 uppercase tracking-wider">Products</p>
                        <div class="w-7 h-7 rounded-lg bg-purple-500/10 border border-purple-500/15 flex items-center justify-center group-hover:bg-purple-500/20 transition-colors">
                            <svg class="w-3.5 h-3.5 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
                        </div>
                    </div>
                    <p class="stat-number text-3xl font-bold text-white">{{ $activeProductsCount }}</p>
                    <p class="text-xs text-slate-500 mt-1">active</p>
                </a>
        ...
        @if($hasProperty)
        <div>
            <h2 class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Property Management</h2>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-slate-800 rounded-xl border border-emerald-800/40 relative overflow-hidden shadow-lg shadow-emerald-950/40">
                    ...
                    <div class="relative p-5">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <p class="text-xs text-slate-400 uppercase tracking-wider">Rent collected this month</p>
                        </div>
                        <p class="stat-number text-4xl font-bold text-emerald-400 leading-none">R{{ number_format($rentCollectedThisMonth, 2) }}</p>
                        <p class="text-xs text-slate-500 mt-2">{{ $activeLeaseCount }} active {{ Str::plural('lease', $activeLeaseCount) }}</p>
                    </div>
                </div>

                <div class="{{ $rentOverdueCount > 0 ? 'bg-red-900/25 border-red-700/50 shadow-red-950/40' : 'bg-slate-800 border-slate-700/50 shadow-black/20' }} rounded-xl border relative overflow-hidden shadow-lg">
                    <div class="absolute top-0 left-0 right-0 h-0.5 {{ $rentOverdueCount > 0 ? 'bg-gradient-to-r from-red-500/60 to-transparent' : 'bg-gradient-to-r from-slate-600/40 to-transparent' }}"></div>
                    <div class="p-5">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-6 h-6 rounded-lg {{ $rentOverdueCount > 0 ? 'bg-red-500/20 border-red-500/30' : 'bg-slate-700 border-slate-600' }} border flex items-center justify-center">
                                <svg class="w-3.5 h-3.5 {{ $rentOverdueCount > 0 ? 'text-red-400' : 'text-slate-500' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
                            </div>
                            <p class="text-xs {{ $rentOverdueCount > 0 ? 'text-red-400' : 'text-slate-400' }} uppercase tracking-wider">Rent overdue</p>
                        </div>
                        <p class="stat-number text-4xl font-bold {{ $rentOverdueCount > 0 ? 'text-red-300' : 'text-slate-600' }} leading-none">R{{ number_format($rentOverdueTotal, 2) }}</p>
                        <p class="text-xs text-slate-500 mt-2">
                            {{ $rentOverdueCount }} {{ Str::plural('payment', $rentOverdueCount) }} overdue
                            @if($rentOverdueCount > 0)
                                · <a href="{{ route('rent-payments.index', ['status' => 'overdue']) }}" class="text-red-400 hover:text-red-300">View →</a>
                            @endif
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-slate-800 rounded-xl border border-slate-700/50 relative overflow-hidden shadow-lg shadow-black/20">
                    ...
                    <div class="p-5">
                        ...
                        <p class="stat-number text-4xl font-bold text-white leading-none">{{ $occupancyRate }}%</p>
                        <p class="text-xs text-slate-500 mt-2">{{ $occupiedUnits }} occupied · {{ $vacantUnits }} vacant of {{ $totalUnits }} {{ Str::plural('unit', $totalUnits) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
                <a href="{{ route('properties.index') }}"
                   class="bg-slate-800 hover:bg-slate-700/70 rounded-xl p-4 transition-all shadow-md shadow-black/20 border border-slate-700/50 border-t-2 border-t-[#0078D4]/40 group">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-xs text-slate-400 uppercase tracking-wider">Properties</p>
                        <div class="w-7 h-7 rounded-lg bg-[#0078D4]/10 border border-[#0078D4]/15 flex items-center justify-center group-hover:bg-[#0078D4]/20 transition-colors">
                            <svg class="w-3.5 h-3.5 text-[#0078D4]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                        </div>
                    </div>
                    <p class="stat-number text-3xl font-bold text-white">{{ $totalProperties }}</p>
                    <p class="text-xs text-slate-500 mt-1">total</p>
                </a>
                ...
                <a href="{{ route('leases.index') }}"
                   class="bg-slate-800 hover:bg-slate-700/70 rounded-xl p-4 transition-all shadow-md shadow-black/20 border border-slate-700/50 border-t-2 border-t-emerald-500/40 group">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-xs text-slate-400 uppercase tracking-wider">Active leases</p>
                        <div class="w-7 h-7 rounded-lg bg-emerald-500/10 border border-emerald-500/15 flex items-center justify-center group-hover:bg-emerald-500/20 transition-colors">
                    ...
                    <p class="stat-number text-3xl font-bold text-white">{{ $activeLeaseCount }}</p>
                    <p class="text-xs text-slate-500 mt-1">active</p>
                </a>

                <a href="{{ route('leases.index') }}"
                   class="{{ $leasesExpiringSoonCount > 0 ? 'bg-yellow-900/25 border-yellow-700/50 border-t-yellow-500/60' : 'bg-slate-800 border-slate-700/50 border-t-slate-600/40' }} hover:bg-slate-700/70 rounded-xl p-4 transition-all shadow-md shadow-black/20 border border-t-2 group">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-xs {{ $leasesExpiringSoonCount > 0 ? 'text-yellow-400' : 'text-slate-400' }} uppercase tracking-wider">Expiring soon</p>
                        <div class="w-7 h-7 rounded-lg {{ $leasesExpiringSoonCount > 0 ? 'bg-yellow-500/20 border-yellow-500/30' : 'bg-slate-700 border-slate-600' }} border flex items-center justify-center">
                            <svg class="w-3.5 h-3.5 {{ $leasesExpiringSoonCount > 0 ? 'text-yellow-400' : 'text-slate-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        </div>
                    </div>
                    <p class="stat-number text-3xl font-bold {{ $leasesExpiringSoonCount > 0 ? 'text-yellow-300' : 'text-slate-600' }}">{{ $leasesExpiringSoonCount }}</p>
                    <p class="text-xs text-slate-500 mt-1">within 30 days</p>
                </a>

                <a href="{{ route('maintenance.index') }}"
                   class="{{ $pendingMaintenanceCount > 0 ? 'bg-yellow-900/25 border-yellow-700/50 border-t-yellow-500/60' : 'bg-slate-800 border-slate-700/50 border-t-slate-600/40' }} hover:bg-slate-700/70 rounded-xl p-4 transition-all shadow-md shadow-black/20 border border-t-2 group">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-xs {{ $pendingMaintenanceCount > 0 ? 'text-yellow-400' : 'text-slate-400' }} uppercase tracking-wider">Maintenance</p>
                        <div class="w-7 h-7 rounded-lg {{ $pendingMaintenanceCount > 0 ? 'bg-yellow-500/20 border-yellow-500/30' : 'bg-slate-700 border-slate-600' }} border flex items-center justify-center">
                            <svg class="w-3.5 h-3.5 {{ $pendingMaintenanceCount > 0 ? 'text-yellow-400' : 'text-slate-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"/></svg>
                        </div>
                    </div>
                    <p class="stat-number text-3xl font-bold {{ $pendingMaintenanceCount > 0 ? 'text-yellow-300' : 'text-slate-600' }}">{{ $pendingMaintenanceCount }}</p>
                    <p class="text-xs text-slate-500 mt-1">{{ $pendingMaintenanceCount > 0 ? 'open requests' : 'all clear' }}</p>
                </a>
            </div>

            @if($leasesExpiringSoon->isNotEmpty())
                <div class="bg-slate-800 rounded-xl overflow-hidden shadow-xl shadow-black/25 border border-slate-700/50 mt-4">
                    <div class="px-4 py-3 border-b border-slate-700 flex items-center justify-between bg-gradient-to-r from-yellow-500/8 to-transparent">
                        <div class="flex items-center gap-2">
                            <div class="w-5 h-5 rounded-md bg-yellow-500/15 border border-yellow-500/20 flex items-center justify-center">
                                <svg class="w-3 h-3 text-yellow-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            </div>
                            <h3 class="text-sm font-semibold text-white">Leases Expiring Soon</h3>
                        </div>
                        <a href="{{ route('leases.index') }}" class="text-xs text-[#0078D4] hover:text-[#0065B8] font-medium">View all →</a>
                    </div>
                    @foreach($leasesExpiringSoon as $lease)
                        <a href="{{ route('leases.show', $lease) }}"
                           class="px-4 py-3 border-b border-slate-700/50 flex items-center gap-3 hover:bg-slate-700/30 transition-colors block last:border-b-0">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm text-white truncate">{{ $lease->renter?->name ?? 'Unassigned' }} · Unit {{ $lease->unit?->unit_number }}</p>
                                <p class="text-xs text-slate-400 truncate">Ends {{ $lease->end_date->format('d M Y') }}</p>
                            </div>
                            <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-900/50 text-yellow-400 border border-yellow-800 shrink-0">
                                {{ (int) now()->diffInDays($lease->end_date, false) }}d left
                            </span>
                        </a>
                    @endforeach
                </div>
        ...
        @if($hasEcommerce)
        <div>
            <h2 class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Online Store</h2>
                ...
                <div class="bg-slate-800 rounded-xl border border-emerald-800/40 relative overflow-hidden shadow-lg shadow-emerald-950/40">
                    ...
                    <div class="relative p-5">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <p class="text-xs text-slate-400 uppercase tracking-wider">Store revenue this month</p>
                        </div>
                        <p class="stat-number text-4xl font-bold text-emerald-400 leading-none">R{{ number_format($ecommerceRevenueThisMonth, 2) }}</p>
                        <p class="text-xs text-slate-500 mt-2">paid orders</p>
                    </div>
                </div>

                <a href="{{ route('orders.index', ['status' => 'pending']) }}"
                   class="{{ $pendingOrdersCount > 0 ? 'bg-amber-900/25 border-amber-700/50 shadow-amber-950/40' : 'bg-slate-800 border-slate-700/50 shadow-black/20' }} rounded-xl border relative overflow-hidden shadow-lg block hover:bg-slate-700/40 transition-colors">
                    <div class="absolute top-0 left-0 right-0 h-0.5 {{ $pendingOrdersCount > 0 ? 'bg-gradient-to-r from-amber-500/60 to-transparent' : 'bg-gradient-to-r from-slate-600/40 to-transparent' }}"></div>
                    <div class="p-5">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-6 h-6 rounded-lg {{ $pendingOrdersCount > 0 ? 'bg-amber-500/20 border-amber-500/30' : 'bg-slate-700 border-slate-600' }} border flex items-center justify-center">
                                <svg class="w-3.5 h-3.5 {{ $pendingOrdersCount > 0 ? 'text-amber-400' : 'text-slate-500' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                            </div>
                            <p class="text-xs {{ $pendingOrdersCount > 0 ? 'text-amber-400' : 'text-slate-400' }} uppercase tracking-wider">Pending orders</p>
                        </div>
                        <p class="stat-number text-4xl font-bold {{ $pendingOrdersCount > 0 ? 'text-amber-300' : 'text-slate-600' }} leading-none">{{ $pendingOrdersCount }}</p>
                        <p class="text-xs text-slate-500 mt-2">{{ $pendingOrdersCount > 0 ? 'need action' : 'all clear' }}</p>
                    </div>
                </a>
            </div>
            ...
            <div class="bg-slate-800 rounded-xl overflow-hidden shadow-xl shadow-black/25 border border-slate-700/50 mt-4">
                <div class="px-4 py-3 border-b border-slate-700 flex items-center justify-between bg-gradient-to-r from-[#D4AF37]/8 to-transparent">
                    ...
                    <a href="{{ route('orders.index') }}" class="text-xs text-[#0078D4] hover:text-[#0065B8] font-medium">View all →</a>
                </div>
                @forelse($recentOrders as $order)
                    <a href="{{ route('orders.show', $order) }}"
                       class="px-4 py-3 border-b border-slate-700/50 flex items-center gap-3 hover:bg-slate-700/30 transition-colors block last:border-b-0">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm text-white truncate">{{ $order->customer_name }} · #{{ $order->reference }}</p>
                            <p class="text-xs text-slate-400 truncate">R{{ number_format($order->total, 2) }} · {{ $order->created_at->format('d M, H:i') }}</p>
                        </div>
                        @php
                            // Written as literal class strings (not interpolated from statusColor()) so
                            // Tailwind's content scanner picks them up — see feedback_tailwind_build_gotcha.
                            $orderBadgeClasses = [
                                'pending'    => 'bg-yellow-900/50 text-yellow-400 border-yellow-800',
                                'paid'       => 'bg-blue-900/50 text-blue-400 border-blue-800',
                                'processing' => 'bg-indigo-900/50 text-indigo-400 border-indigo-800',
                                'ready'      => 'bg-purple-900/50 text-purple-400 border-purple-800',
                                'shipped'    => 'bg-cyan-900/50 text-cyan-400 border-cyan-800',
                            ...
                            $obc = $orderBadgeClasses[$order->status] ?? 'bg-slate-700 text-slate-400 border-slate-600';
                        @endphp
                        <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium border {{ $obc }} shrink-0">
                            {{ $order->statusLabel() }}
                        </span>
                    </a>
                @empty
                    <div class="px-4 py-8 text-center text-slate-500 text-sm">No orders yet.</div>
                @endforelse
            </div>
        </div>
        @endif
        ...
        @if($hasMessaging)
        <div>
            <h2 class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Client Messaging</h2>

            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <a href="{{ route('clients.index') }}"
                   class="{{ $unreadMessagesCount > 0 ? 'bg-amber-900/25 border-amber-700/50 border-t-amber-500/60' : 'bg-slate-800 border-slate-700/50 border-t-slate-600/40' }} hover:bg-slate-700/70 rounded-xl p-4 transition-all shadow-md shadow-black/20 border border-t-2 group">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-xs {{ $unreadMessagesCount > 0 ? 'text-amber-400' : 'text-slate-400' }} uppercase tracking-wider">Unread</p>
                        <div class="w-7 h-7 rounded-lg {{ $unreadMessagesCount > 0 ? 'bg-amber-500/20 border-amber-500/30' : 'bg-slate-700 border-slate-600' }} border flex items-center justify-center">
                            <svg class="w-3.5 h-3.5 {{ $unreadMessagesCount > 0 ? 'text-amber-400' : 'text-slate-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
                        </div>
                    </div>
                    <p class="stat-number text-3xl font-bold {{ $unreadMessagesCount > 0 ? 'text-amber-300' : 'text-slate-600' }}">{{ $unreadMessagesCount }}</p>
                    <p class="text-xs text-slate-500 mt-1">{{ $unreadMessagesCount > 0 ? 'messages' : 'all read' }}</p>
                </a>

                <a href="{{ route('clients.index') }}"
                   class="bg-slate-800 hover:bg-slate-700/70 rounded-xl p-4 transition-all shadow-md shadow-black/20 border border-slate-700/50 border-t-2 border-t-[#D4AF37]/40 group">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-xs text-slate-400 uppercase tracking-wider">Clients</p>
                        <div class="w-7 h-7 rounded-lg bg-[#D4AF37]/10 border border-[#D4AF37]/15 flex items-center justify-center group-hover:bg-[#D4AF37]/20 transition-colors">
                            <svg class="w-3.5 h-3.5 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a4 4 0 00-4-4h-3m-4 6H7a4 4 0 01-4-4v-2a4 4 0 014-4h1m4 0a4 4 0 100-8 4 4 0 000 8z"/></svg>
                        </div>
                    </div>
                    <p class="stat-number text-3xl font-bold text-white">{{ $totalClients }}</p>
                    <p class="text-xs text-slate-500 mt-1">total</p>
                </a>
            </div>

            @if($recentCommunications->isNotEmpty())
                <div class="bg-slate-800 rounded-xl overflow-hidden shadow-xl shadow-black/25 border border-slate-700/50 mt-4">
                    <div class="px-4 py-3 border-b border-slate-700 flex items-center gap-2 bg-gradient-to-r from-[#0078D4]/10 to-transparent">
                        <div class="w-5 h-5 rounded-md bg-[#0078D4]/20 border border-[#0078D4]/25 flex items-center justify-center">
                            <svg class="w-3 h-3 text-[#0078D4]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
                        </div>
                        <h3 class="text-sm font-semibold text-white">Recent Messages</h3>
                    </div>
                    @foreach($recentCommunications as $comm)
                        <a href="{{ route('clients.messages', $comm->client_id) }}"
                           class="px-4 py-3 border-b border-slate-700/50 flex items-center gap-3 hover:bg-slate-700/30 transition-colors block last:border-b-0">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm text-white truncate">{{ $comm->client?->name ?? 'Unknown client' }}</p>
                                <p class="text-xs text-slate-400 truncate">{{ Str::limit($comm->body, 60) }}</p>
                            </div>
                            <span class="text-xs text-slate-500 shrink-0">{{ $comm->created_at->diffForHumans() }}</span>
                        </a>
                    @endforeach
                </div>
            @endif
