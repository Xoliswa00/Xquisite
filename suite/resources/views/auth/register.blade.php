        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            <div x-show="step === 1" x-cloak class="space-y-4">
                ...
                <div>
                    ...
                    <select id="industry" name="industry" x-model="industry"
                ...
                <div id="industry_other_field" x-show="industry === 'other'" x-cloak>
                ...
                @if ($templates->isNotEmpty())
                    <button type="button" @click="step = 2"
                            class="w-full bg-[#0078D4] hover:bg-[#0065B8] text-white font-semibold py-3 rounded-xl text-sm transition-colors mt-2">
            ...
            {{-- ── Step 2: Website template (optional) ── --}}
            @if ($templates->isNotEmpty())
            <div x-show="step === 2" x-cloak class="space-y-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <button type="button" @click="filter = 'all'"
                            ...
                            :class="filter === 'all' ? 'bg-[#0078D4] border-[#0078D4] text-white' : 'border-gray-300 text-gray-600'">
                        All
                    </button>
                    @foreach ($templates->pluck('category')->unique()->filter() as $category)
                        <button type="button" @click="filter = '{{ $category }}'"
                                class="px-3 py-1 rounded-full text-xs font-medium border capitalize transition-colors"
                                :class="filter === '{{ $category }}' ? 'bg-[#0078D4] border-[#0078D4] text-white' : 'border-gray-300 text-gray-600'">
                ...
                <div class="space-y-3 max-h-80 overflow-y-auto pr-1">
                    @foreach ($templates as $t)
                        <button type="button" x-show="filter === 'all' || filter === '{{ $t->category }}'"
                                @click="templateKey = (templateKey === '{{ $t->key }}') ? '' : '{{ $t->key }}'"
                                class="w-full flex items-center gap-3 rounded-xl border p-2 text-left transition-colors"
                                :class="templateKey === '{{ $t->key }}' ? 'border-[#0078D4] bg-[#0078D4]/5' : 'border-gray-200 hover:border-gray-300'">
                            <x-template-preview :template="$t" ratio="aspect-video" class="w-24 shrink-0 border border-gray-200" />
                ...
                <div class="flex gap-3 pt-2">
                    <button type="button" @click="step = 1" class="px-4 py-3 text-sm text-gray-500 hover:text-[#002B5B] transition-colors">
                        &larr; Back
                    </button>
                    <button type="submit" data-loading="Creating your account…"
