<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl font-bold text-[#D4AF37]">{{ $module->name }}</h2>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('admin.platform-modules.update', $module) }}" class="bg-slate-800 rounded-xl border border-slate-700 divide-y divide-slate-700">
            @csrf @method('PUT')
