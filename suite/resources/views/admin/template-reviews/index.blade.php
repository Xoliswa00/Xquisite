<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-[#D4AF37]">Template Reviews</h2>
    </x-slot>
    ...
    <div class="space-y-8">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-3 flex-wrap">
                                <span class="text-amber-400 font-bold tracking-tight text-sm">{{ $review->starsHtml() }}</span>
                                <span class="text-xs px-1.5 py-0.5 rounded bg-slate-700 text-slate-300 border border-slate-600">{{ $review->template?->name ?? $review->template_key }}</span>
                                @if ($review->title)
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-2 text-xs text-slate-500">
                                <span>{{ $review->tenant?->name ?? 'Unknown tenant' }}</span>
                                <span>·</span><span>{{ $review->created_at->diffForHumans() }}</span>
                            </div>
                        </div>

                        <div class="flex items-center gap-2 flex-wrap shrink-0">
                            <form method="POST" action="{{ route('admin.template-reviews.status', $review) }}">
                                @csrf @method('PATCH')
        ...
        @if ($reviews->isEmpty())
            <div class="text-center py-16 text-slate-500">
                <p>No template reviews yet.</p>
                <p class="text-sm mt-1 text-slate-600">Reviews appear here once a tenant who has activated a template rates it.</p>
            </div>
        @endif
