<x-app-layout>
    ...
    <div class="py-8 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8"
        ...
        @if ($errors->any())
            <div class="mb-6 p-4 bg-red-900/50 border border-red-700 text-red-300 rounded-lg text-sm space-y-1">
        ...
        <form method="POST" action="{{ route('quotes.store') }}" class="space-y-5">
            ...
            <div class="bg-slate-800 rounded-xl border border-slate-700 p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-1">Deposit Required (%)</label>
            ...
            <div class="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-700/50 flex items-center justify-between">
                    ...
                    <button type="button" @click="addItem()"
                            class="text-xs text-[#0078D4] hover:text-[#002B5B] font-medium">+ Add line</button>
                </div>
                    ...
                    <template x-for="(item, i) in items" :key="i">
                        <div class="flex items-center gap-2">
                            ...
                            <input type="number" x-model.number="item.qty" :name="'line_items['+i+'][qty]'" min="0" step="0.5"
                                   ...
                                   class="w-20 border border-slate-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0078D4]">
                            <input type="text" x-model="item.unit" :name="'line_items['+i+'][unit]'" placeholder="pax/hr/item"
                                   class="w-20 border border-slate-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0078D4]">
                            <div class="relative w-28">
                                <span class="absolute inset-y-0 left-2.5 flex items-center text-slate-500 text-sm">R</span>
                                <input type="number" x-model.number="item.unit_price" :name="'line_items['+i+'][unit_price]'" min="0" step="0.01"
                                       ...
                                       class="w-full border border-slate-600 rounded-lg pl-6 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0078D4]">
                            </div>
                            <input type="hidden" :name="'line_items['+i+'][total]'" :value="item.total">
                            <p class="w-24 text-sm font-medium text-white text-right" x-text="'R ' + item.total.toFixed(2)"></p>
                            <button type="button" @click="removeItem(i)"
                                    class="text-slate-500 hover:text-red-400 transition" x-show="items.length > 1">
                ...
                <div class="px-5 pb-5 space-y-2 border-t border-slate-700/50 pt-4">
                    ...
                    <div class="flex items-center justify-between text-sm text-slate-500">
                        <span class="flex items-center gap-2">
                            ...
                            <input type="number" name="tax_rate" min="0" max="100" step="1"
                                   ...
                                   class="w-16 border border-slate-700 rounded px-2 py-0.5 text-xs focus:outline-none focus:ring-1 focus:ring-[#0078D4]">
                    ...
                    <div class="flex justify-between text-sm text-[#0078D4] font-medium">
            ...
            <div class="bg-slate-800 rounded-xl border border-slate-700 p-5">
                <label class="block text-sm font-medium text-slate-300 mb-1">Notes / Terms</label>
                <textarea name="notes" rows="3" placeholder="Payment terms, cancellation policy, inclusions/exclusions..."
                          class="w-full border border-slate-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0078D4] resize-none">{{ old('notes') }}</textarea>
            </div>

            <div class="flex justify-end gap-3">
                <a href="{{ route('quotes.index') }}" class="px-4 py-2 text-sm text-slate-400 hover:text-slate-100">Cancel</a>
                <button type="submit" class="px-5 py-2 bg-[#0078D4] hover:bg-[#0078D4] text-white text-sm rounded-lg font-medium">
                    Save Quote
                </button>
            </div>
