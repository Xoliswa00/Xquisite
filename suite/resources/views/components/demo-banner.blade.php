...
@php
    ...
    $wa  = config('contact.whatsapp_number');
    $msg = urlencode(config('contact.whatsapp_message'));
@endphp

<div x-data="{ open: true }" x-show="open"
     class="bg-indigo-600 text-white text-sm px-4 py-2.5 relative z-50">
    <div class="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-4 flex-wrap">
            ...
            <span class="hidden sm:flex items-center gap-1 text-indigo-200 text-xs">
                Explore:
            </span>
            ...
            <div class="hidden sm:flex items-center gap-1">
                @foreach ($steps as $step)
                    ...
                    <a href="{{ route($step['route']) }}"
                       class="flex items-center gap-1 px-2.5 py-1 rounded-lg hover:bg-white/15 transition text-xs
                              {{ request()->routeIs(str_replace('.index', '.*', $step['route'])) ? 'bg-white/20 font-medium' : 'text-indigo-100' }}">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="https://wa.me/{{ $wa }}?text={{ $msg }}" target="_blank" rel="noopener"
               class="hidden sm:flex items-center gap-1.5 text-xs text-indigo-200 hover:text-white transition">
            ...
            <a href="{{ route('register') }}"
               class="flex items-center gap-1 bg-white text-indigo-700 hover:bg-indigo-50 text-xs font-semibold px-3 py-1.5 rounded-lg transition shrink-0">
                Create free account →
            </a>

            <button @click="open = false" class="text-indigo-300 hover:text-white ml-1" aria-label="Dismiss">
