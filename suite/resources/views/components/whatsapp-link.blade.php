...
@php
    ...
    $url = $digits ? 'https://wa.me/' . $digits . ($message ? '?text=' . rawurlencode($message) : '') : null;
@endphp
@if(!empty($phone))
    @if($url)
        <a href="{{ $url }}" target="_blank" rel="noopener" title="Chat on WhatsApp"
           {{ $attributes->merge(['class' => 'inline-flex items-center gap-1.5 hover:text-[#25D366] transition-colors']) }}>
            ...
            <span>{{ $slot->isEmpty() ? $phone : $slot }}</span>
        </a>
    @else
        <span {{ $attributes }}>{{ $slot->isEmpty() ? $phone : $slot }}</span>
    @endif
@endif
