<x-app-layout>
    <x-slot name="header">Edit — {{ $property->name }}</x-slot>
    ...
    <div class="max-w-2xl mx-auto p-6">
        <form method="POST" action="{{ route('properties.update', $property) }}" class="space-y-6">
            ...
            <div class="bg-slate-800 rounded-xl p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type" class="w-full bg-slate-700 border-slate-600 text-slate-100 rounded-lg text-sm px-3 py-2">
                    ...
                    <div class="flex items-end">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $property->is_active)) class="rounded border-slate-600">
                ...
                <div>
                    ...
                    <input type="text" name="address_line_1" value="{{ old('address_line_1', $property->address_line_1) }}" required
                           class="w-full bg-slate-700 border-slate-600 text-slate-100 rounded-lg text-sm px-3 py-2">
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1">Address Line 2</label>
                    <input type="text" name="address_line_2" value="{{ old('address_line_2', $property->address_line_2) }}"
                           class="w-full bg-slate-700 border-slate-600 text-slate-100 rounded-lg text-sm px-3 py-2">
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full bg-slate-700 border-slate-600 text-slate-100 rounded-lg text-sm px-3 py-2">{{ old('description', $property->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-slate-800 rounded-xl p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="owner_id_number" value="{{ old('owner_id_number', $property->owner_id_number) }}"
                               class="w-full bg-slate-700 border-slate-600 text-slate-100 rounded-lg text-sm px-3 py-2">
            ...
            <div class="bg-slate-800 rounded-xl p-6 space-y-4">
                <h3 class="text-sm font-semibold text-slate-300">Lease Settings</h3>
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-slate-400 mb-1">Annual Increase (%)</label>
                        <input type="number" name="annual_increase_percentage" step="0.01" min="0" max="100"
                               value="{{ old('annual_increase_percentage', $property->annual_increase_percentage) }}" placeholder="e.g. 10"
                               class="w-full bg-slate-700 border-slate-600 text-slate-100 rounded-lg text-sm px-3 py-2">
                        <p class="text-xs text-slate-500 mt-1">Suggested rent increase applied when renewing a lease on this property.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-between">
                ...
                <button type="submit" class="px-5 py-2 bg-[#0078D4] hover:bg-[#0065B8] text-white rounded-lg text-sm font-semibold">Save Changes</button>
            </div>
        </form>
    </div>
</x-app-layout>
