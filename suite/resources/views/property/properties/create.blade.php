<x-app-layout>
    <x-slot name="header">Add Property</x-slot>
    ...
    <div class="max-w-2xl mx-auto p-6">
        <form method="POST" action="{{ route('properties.store') }}" class="space-y-6" enctype="multipart/form-data">
            ...
            <div class="bg-slate-800 rounded-xl p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1">Upload Photos</label>
                    <input type="file" name="photos[]" multiple accept="image/png,image/jpeg,image/webp"
                           class="w-full text-sm text-slate-300 file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-slate-700 file:text-slate-200 file:text-sm hover:file:bg-slate-600">
                    <p class="text-xs text-slate-500 mt-1">Optional — you can add or remove photos later from the property page.</p>
                </div>
            </div>
            ...
            <div class="bg-slate-800 rounded-xl p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="owner_id_number" value="{{ old('owner_id_number') }}"
                               class="w-full bg-slate-700 border-slate-600 text-slate-100 rounded-lg text-sm px-3 py-2">
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="number" name="annual_increase_percentage" step="0.01" min="0" max="100"
                               value="{{ old('annual_increase_percentage') }}" placeholder="e.g. 10"
                               class="w-full bg-slate-700 border-slate-600 text-slate-100 rounded-lg text-sm px-3 py-2">
